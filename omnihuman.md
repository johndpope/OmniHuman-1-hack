Below is a functional specification for recreating the OmniHuman model described in the paper "OmniHuman-1: Rethinking the Scaling-Up of One-Stage Conditioned Human Animation Models" by Gaojie Lin et al. The spec is broken into separate components without including actual code, focusing on the high-level design, functionality, and interactions as outlined in the paper. It is structured to reflect the key elements of the OmniHuman framework: model architecture, training strategy, data handling, and inference process.
